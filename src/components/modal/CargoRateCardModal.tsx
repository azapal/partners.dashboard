...
import type { CargoRateCard, RateCardStatus } from '../../service/partnerService';
import { useCreateCargoRate, useUpdateCargoRate, useDeleteCargoRate } from '../../hooks/useRateCards';

interface CargoRateCardModalProps {
  rate?: CargoRateCard | null;
  onClose: () => void;
}
...
export const CargoRateCardModal: React.FC<CargoRateCardModalProps> = ({ rate, onClose }) => {
  const isEdit = !!rate;
  const [cargoType, setCargoType] = useState<CargoType>(rate?.cargo_type ?? 'General Goods');
  const [basePrice, setBasePrice] = useState(rate ? String(rate.base_price) : '');
  const [pricePerKg, setPricePerKg] = useState(rate ? String(rate.price_per_kg) : '');
  ...
  const canSubmit = Number(basePrice) >= 1 && Number(pricePerKg) >= 0;
  ...
  const handleDelete = () => {
    if (!rate) return;
    if (window.confirm('Delete this cargo rate? This cannot be undone.')) {
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <FormField label="Base Price (₦)" required>
            <input
              type="number" min={1} step="any" value={basePrice} onChange={(e) => setBasePrice(e.target.value)}
              placeholder="e.g., 5000" className={inputClass}
            />
          </FormField>
          <FormField label="Price per KG (₦)" required>
            <input
              type="number" min={0} step="any" value={pricePerKg} onChange={(e) => setPricePerKg(e.target.value)}
